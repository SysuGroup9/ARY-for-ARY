...
test("removeJudgeAssignmentAction follows managed-race organizer and system admin boundaries", () => {
  ...
  assert.match(
    functionSource,
    /await removeJudgeAssignment\(\{[\s\S]*?allowSystem: hasRole\(user\.roles, "ADMIN"\)[\s\S]*?assignedByUserId: user\.id[\s\S]*?\}\)/,
  );
});
