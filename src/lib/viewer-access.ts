...
export function getRoleCapabilities(roles: readonly AppRole[] | null): {
  ...
  return {
    ...
    canUseScreen: hasRole(normalized, "ADMIN") || hasRole(normalized, "ORGANIZER"),
  };
}
...
export function getConsoleHomeSections(
  ...
  if (hasRole(normalized, "ADMIN") || hasRole(normalized, "ORGANIZER")) {
    sections.add("screen");
  }
...
export function getConsoleScreenAccess(roles: readonly AppRole[] | null): {
  ...
  const allowed = hasRole(normalized, "ADMIN") || hasRole(normalized, "ORGANIZER");
